# 줍줍 (JubJub) 📍

> "발길이 닿는 곳에 당신의 흔적을 남기세요."

특정 위치에 디지털 노트를 남기고, 해당 장소를 방문한 사람이 발견할 수 있는 **위치 기반 소셜 네트워킹 서비스**입니다.  

---

# 📖 소개 (Overview)

**줍줍(JubJub)** 은 단순한 기록 앱이 아니라 **장소에 의미를 남기는 서비스**입니다.

사용자는 현재 위치의 **GPS 좌표**를 기반으로 노트를 지도에 남길 수 있습니다.  

이 서비스는 다음과 같은 상황에서 활용됩니다.

- 동네에서 숨겨진 메시지 남기기
- 여행지에서의 기록 공유
- 특정 장소에서만 볼 수 있는 이야기 전달

현실 공간과 디지털 메모를 연결하는 **위치 기반 커뮤니티 서비스**를 목표로 합니다.

---

# ✨ 주요 기능 (Key Features)

## 📝 쪽지 남기기
추가예정

## 📍 쪽지 줍기
추가예정

## 📦 내 보관함
- 내가 작성한 쪽지
- 내가 주운 쪽지

쪽지 기록을 관리할 수 있습니다.

## 🗺 지도 뷰
추가예정

---

# 🛠 기술 스택 (Tech Stack)

## Language
- Kotlin

## UI Framework
- Jetpack Compose

## Asynchronous
- Coroutines
- Flow

## Architecture
- MVVM

## Database


## API


---

# 📱 앱 구조 (Example Architecture)

```
추가예정
```

---

# 🚀 향후 확장 기능 (Future Plans)
- 추가예정
